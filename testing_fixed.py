...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        #s    plot_images(img, output_to_target(output, width, height), paths, f, names)  # predictions
    ...
    print(t0_list)
    print([t0_list[i] - t0_list[i-1] for i in range(1, len(t0_list))])
    print(np.mean([t0_list[i] - t0_list[i-1] for i in range(1, len(t0_list))]))
    if not training:
