...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    save_dir, epochs, batch_size, total_batch_size, weights, rank, tlpretrain = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.total_batch_size, opt.weights, opt.global_rank, opt.pretrain
    ...
    elif tlpretrain.endswith('.pth') or tlpretrain.endswith('.pt'):
        model = Darknet(opt.cfg).to(device) # create
        print('Loading pretrained weights from %s' % (tlpretrain))
        model = load_swin_weights(model, tlpretrain)
    else:
        model = Darknet(opt.cfg).to(device) # create
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrain', default='', help='path to pretrained weights')
