...
def create_modules(module_defs, img_size, cfg):
    ...
    train_hyp = module_defs.pop(0)  # cfg training hyperparams (unused)
    ...
    for i, mdef in enumerate(module_defs):
        ...
        elif mdef['type'] == 'patchEmbed':
            # revisar uso de filters
            #filters = 4 * output_filters[-1]
            
            img_size=mdef['img_size']
            patch_size=mdef['patch_size']
            in_chans=mdef['in_chans']
            embed_dim=mdef['embed_dim']
            patch_norm=mdef['patch_norm']
            norm_layer=nn.LayerNorm
            
            filters = embed_dim
            
            modules = PatchEmbed(
                    patch_size=patch_size, in_chans=output_filters[-1], embed_dim=embed_dim,
...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg, img_size=(416, 416), verbose=False):
        ...
        embed_dim = 96 # REEMPLAZAR
        num_layers = 2
        num_features = [int(embed_dim * 2 ** i) for i in range(num_layers)]
