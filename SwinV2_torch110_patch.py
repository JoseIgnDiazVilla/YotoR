...
class _SwinTransformer(nn.Module):
    ...
    def forward(self, x): # cambiar esto para q entregue una lista
        x = self.features(x)
        x = self.norm(x)
        x = self.permute(x)
        x = self.avgpool(x)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        layers: List[nn.Module] = []
        # split image into non-overlapping patches
        self.patch_embedd = nn.Sequential(
                nn.Conv2d(
                    3, embed_dim, kernel_size=(patch_size[0], patch_size[1]), stride=(patch_size[0], patch_size[1])
                ),
                Permute([0, 2, 3, 1]),
    ...
    def forward(self, x): # cambiar esto para q entregue una lista
        ...
        for i in range(len(self.stages) - 1):
            ...
            x = stage(x)
            # out = x.view(-1, H, W, self.num_features[i]).permute(0, 3, 1, 2).contiguous()
